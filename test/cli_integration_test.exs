defmodule RPCProtoGenHelpers.CLITest do
  use ExUnit.Case

  describe "generates Elixir helper modules" do
    setup :tmp_dir

    test "legacy style package - impl and behaviour modules are generated correctly", %{
      tmp_dir: tmp_dir
    } do
      rpc_path = Path.join(tmp_dir, "rpc/gift_cards/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      import "rpc/gift_cards/v1/initialise_gift_card.proto";

      service RPCService {
        rpc InitialiseGiftCard(rpc.gift_cards.v1.InitialiseGiftCardRequest) returns (rpc.gift_cards.v1.InitialiseGiftCardResponse);
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      message InitialiseGiftCardRequest {}

      message InitialiseGiftCardResponse {
        int32 id = 1;
      }
      """

      File.write!(Path.join(rpc_path, "gift_cards_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "initialise_gift_card.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      expected_impl = ~s"""
      defmodule Rpc.GiftCards.V1.RPCService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.GiftCards.V1.RPCService.ClientBehaviour
        use RpcClient.Client, service: Rpc.GiftCards.V1.RPCService, stub: Rpc.GiftCards.V1.RPCService.Stub

        @impl true
        @doc \"\"\"
        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec initialise_gift_card(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.GiftCards.V1.InitialiseGiftCardResponse.t()} | {:error, GRPC.RPCError.t()}
        def initialise_gift_card(request, opts \\\\ []) do
          call(request, :initialise_gift_card, opts)
        end

        @impl true
        @spec initialise_gift_card!(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.GiftCards.V1.InitialiseGiftCardResponse.t() | no_return()
        def initialise_gift_card!(request, opts \\\\ []) do
          call!(request, :initialise_gift_card, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/gift_cards/v1/gift_cards_service_client_impl.ex"
      )

      expected_behaviour = ~S"""
      defmodule Rpc.GiftCards.V1.RPCService.ClientBehaviour do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @type call_options :: RpcClient.ClientBehaviour.call_options()

        @callback initialise_gift_card(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), call_options()) ::
                          {:ok, Rpc.GiftCards.V1.InitialiseGiftCardResponse.t()}
                          | {:error, GRPC.RPCError.t()}

        @callback initialise_gift_card!(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), call_options()) ::
                          Rpc.GiftCards.V1.InitialiseGiftCardResponse.t() | no_return()
      end
      """

      assert_content_matches_file(
        expected_behaviour,
        "#{tmp_dir}/generated/rpc/gift_cards/v1/gift_cards_service_client_behaviour.ex"
      )
    end

    test "if exclude_packages is set, it does not generate the service(s)", %{
      tmp_dir: tmp_dir
    } do
      a_rpc_path = Path.join(tmp_dir, "rpc/a/v1")
      b_rpc_path = Path.join(tmp_dir, "rpc/b/v1")
      c_rpc_path = Path.join(tmp_dir, "rpc/c/v1")

      File.mkdir_p!(a_rpc_path)
      File.mkdir_p!(b_rpc_path)
      File.mkdir_p!(c_rpc_path)

      a_service_proto = """
      syntax = "proto3";

      package rpc.a.v1;

      import "rpc/a/v1/a_request.proto";

      service RPCService {
        rpc ARequest(rpc.a.v1.ARequest) returns (rpc.a.v1.AResponse);
      }
      """

      b_service_proto = """
      syntax = "proto3";

      package rpc.b.v1;

      import "rpc/b/v1/b_request.proto";

      service RPCService {
        rpc BRequest(rpc.b.v1.BRequest) returns (rpc.b.v1.BResponse);
      }
      """

      c_service_proto = """
      syntax = "proto3";

      package rpc.c.v1;

      import "rpc/c/v1/c_request.proto";

      service RPCService {
        rpc CRequest(rpc.c.v1.CRequest) returns (rpc.c.v1.CResponse);
      }
      """

      a_rpc_proto = """
      syntax = "proto3";

      package rpc.a.v1;
      message ARequest {}
      message AResponse {}
      """

      b_rpc_proto = """
      syntax = "proto3";

      package rpc.b.v1;
      message BRequest {}
      message BResponse {}
      """

      c_rpc_proto = """
      syntax = "proto3";

      package rpc.c.v1;
      message CRequest {}
      message CResponse {}
      """

      File.write!(Path.join(a_rpc_path, "a_service.proto"), a_service_proto)
      File.write!(Path.join(b_rpc_path, "b_service.proto"), b_service_proto)
      File.write!(Path.join(c_rpc_path, "c_service.proto"), c_service_proto)

      File.write!(Path.join(a_rpc_path, "a_request.proto"), a_rpc_proto)
      File.write!(Path.join(b_rpc_path, "b_request.proto"), b_rpc_proto)
      File.write!(Path.join(c_rpc_path, "c_request.proto"), c_rpc_proto)

      exec_buf!(tmp_dir, template: "buf.with.opt.yaml")

      refute File.exists?("#{tmp_dir}/generated/rpc/a/v1/a_service_client_impl.ex")

      refute File.exists?("#{tmp_dir}/generated/rpc/b/v1/b_service_client_impl.ex")

      assert File.exists?("#{tmp_dir}/generated/rpc/c/v1/c_service_client_impl.ex")
    end

    test "injects single-line comments into docs", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "rpc/gift_cards/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      import "rpc/gift_cards/v1/initialise_gift_card.proto";

      service RPCService {
        // this is a comment
        // and
        //
        // this is another
        rpc InitialiseGiftCard(rpc.gift_cards.v1.InitialiseGiftCardRequest) returns (rpc.gift_cards.v1.InitialiseGiftCardResponse);
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      message InitialiseGiftCardRequest {}

      message InitialiseGiftCardResponse {
        int32 id = 1;
      }
      """

      File.write!(Path.join(rpc_path, "gift_cards_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "initialise_gift_card.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      # The extra space is an artifact of the template and will be removed after
      #  formatting
      expected_impl = ~s"""
      defmodule Rpc.GiftCards.V1.RPCService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.GiftCards.V1.RPCService.ClientBehaviour
        use RpcClient.Client, service: Rpc.GiftCards.V1.RPCService, stub: Rpc.GiftCards.V1.RPCService.Stub

        @impl true
        @doc \"\"\"
        this is a comment
        and

        this is another

        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec initialise_gift_card(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.GiftCards.V1.InitialiseGiftCardResponse.t()} | {:error, GRPC.RPCError.t()}
        def initialise_gift_card(request, opts \\\\ []) do
          call(request, :initialise_gift_card, opts)
        end

        @impl true
        @spec initialise_gift_card!(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.GiftCards.V1.InitialiseGiftCardResponse.t() | no_return()
        def initialise_gift_card!(request, opts \\\\ []) do
          call!(request, :initialise_gift_card, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/gift_cards/v1/gift_cards_service_client_impl.ex"
      )

      # Behaviour is unchanged — no retry docs in callbacks
    end

    test "injects multi-line comments into docs", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "rpc/gift_cards/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      import "rpc/gift_cards/v1/initialise_gift_card.proto";

      service RPCService {
        /*
        * I am a
        * multi-line
        *
        * comment
        */
        rpc InitialiseGiftCard(rpc.gift_cards.v1.InitialiseGiftCardRequest) returns (rpc.gift_cards.v1.InitialiseGiftCardResponse);
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      message InitialiseGiftCardRequest {}

      message InitialiseGiftCardResponse {
        int32 id = 1;
      }
      """

      File.write!(Path.join(rpc_path, "gift_cards_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "initialise_gift_card.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      # The extra space is an artifact of the template and will be removed after
      #  formatting
      expected_impl = ~s"""
      defmodule Rpc.GiftCards.V1.RPCService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.GiftCards.V1.RPCService.ClientBehaviour
        use RpcClient.Client, service: Rpc.GiftCards.V1.RPCService, stub: Rpc.GiftCards.V1.RPCService.Stub

        @impl true
        @doc \"\"\"
        I am a
        multi-line

        comment

        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec initialise_gift_card(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.GiftCards.V1.InitialiseGiftCardResponse.t()} | {:error, GRPC.RPCError.t()}
        def initialise_gift_card(request, opts \\\\ []) do
          call(request, :initialise_gift_card, opts)
        end

        @impl true
        @spec initialise_gift_card!(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.GiftCards.V1.InitialiseGiftCardResponse.t() | no_return()
        def initialise_gift_card!(request, opts \\\\ []) do
          call!(request, :initialise_gift_card, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/gift_cards/v1/gift_cards_service_client_impl.ex"
      )

      # Behaviour is unchanged — no retry docs in callbacks
    end

    test "injects multi-line comments without leading asterisks into docs", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "rpc/gift_cards/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      import "rpc/gift_cards/v1/initialise_gift_card.proto";

      service RPCService {
        /*
          I am a
          multi-line

          comment
        */
        rpc InitialiseGiftCard(rpc.gift_cards.v1.InitialiseGiftCardRequest) returns (rpc.gift_cards.v1.InitialiseGiftCardResponse);
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.gift_cards.v1;

      message InitialiseGiftCardRequest {}

      message InitialiseGiftCardResponse {
        int32 id = 1;
      }
      """

      File.write!(Path.join(rpc_path, "gift_cards_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "initialise_gift_card.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      expected_impl = ~s"""
      defmodule Rpc.GiftCards.V1.RPCService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.GiftCards.V1.RPCService.ClientBehaviour
        use RpcClient.Client, service: Rpc.GiftCards.V1.RPCService, stub: Rpc.GiftCards.V1.RPCService.Stub

        @impl true
        @doc \"\"\"
        I am a
        multi-line

        comment

        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec initialise_gift_card(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.GiftCards.V1.InitialiseGiftCardResponse.t()} | {:error, GRPC.RPCError.t()}
        def initialise_gift_card(request, opts \\\\ []) do
          call(request, :initialise_gift_card, opts)
        end

        @impl true
        @spec initialise_gift_card!(Rpc.GiftCards.V1.InitialiseGiftCardRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.GiftCards.V1.InitialiseGiftCardResponse.t() | no_return()
        def initialise_gift_card!(request, opts \\\\ []) do
          call!(request, :initialise_gift_card, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/gift_cards/v1/gift_cards_service_client_impl.ex"
      )
    end

    test "generates modules for services with packages starting with fresha", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "fresha/auth/protobuf/rpc/partners/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package fresha.auth.protobuf.rpc.partners.v1;

      import "fresha/auth/protobuf/rpc/partners/v1/delete_session_hook.proto";

      service PartnersAuthService {
        rpc DeleteSessionHook(DeleteSessionHookRequest) returns (DeleteSessionHookResponse);
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package fresha.auth.protobuf.rpc.partners.v1;

      message DeleteSessionHookRequest {}

      message DeleteSessionHookResponse {
        bool did_remove_session = 1;
      }
      """

      File.write!(Path.join(rpc_path, "partners_auth_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "delete_session_hook.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      expected_impl = ~s"""
      defmodule Fresha.Auth.Protobuf.Rpc.Partners.V1.PartnersAuthService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Fresha.Auth.Protobuf.Rpc.Partners.V1.PartnersAuthService.ClientBehaviour
        use RpcClient.Client, service: Fresha.Auth.Protobuf.Rpc.Partners.V1.PartnersAuthService, stub: Fresha.Auth.Protobuf.Rpc.Partners.V1.PartnersAuthService.Stub

        @impl true
        @doc \"\"\"
        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec delete_session_hook(Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookResponse.t()} | {:error, GRPC.RPCError.t()}
        def delete_session_hook(request, opts \\\\ []) do
          call(request, :delete_session_hook, opts)
        end

        @impl true
        @spec delete_session_hook!(Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookResponse.t() | no_return()
        def delete_session_hook!(request, opts \\\\ []) do
          call!(request, :delete_session_hook, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/fresha/auth/protobuf/rpc/partners/v1/partners_auth_service_client_impl.ex"
      )

      expected_behaviour = ~S"""
      defmodule Fresha.Auth.Protobuf.Rpc.Partners.V1.PartnersAuthService.ClientBehaviour do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @type call_options :: RpcClient.ClientBehaviour.call_options()

        @callback delete_session_hook(Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookRequest.t(), call_options()) ::
                          {:ok, Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookResponse.t()}
                          | {:error, GRPC.RPCError.t()}

        @callback delete_session_hook!(Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookRequest.t(), call_options()) ::
                          Fresha.Auth.Protobuf.Rpc.Partners.V1.DeleteSessionHookResponse.t() | no_return()
      end
      """

      assert_content_matches_file(
        expected_behaviour,
        "#{tmp_dir}/generated/fresha/auth/protobuf/rpc/partners/v1/partners_auth_service_client_behaviour.ex"
      )
    end

    test "extracts idempotency_level and generates idempotent_methods/0", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "rpc/customers/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.customers.v1;

      import "rpc/customers/v1/messages.proto";

      service CustomerService {
        // Gets a customer by ID.
        rpc GetCustomer(rpc.customers.v1.GetCustomerRequest) returns (rpc.customers.v1.GetCustomerResponse) {
          option idempotency_level = NO_SIDE_EFFECTS;
        }

        // Creates a new customer.
        rpc CreateCustomer(rpc.customers.v1.CreateCustomerRequest) returns (rpc.customers.v1.CreateCustomerResponse);

        // Deletes a customer.
        rpc DeleteCustomer(rpc.customers.v1.DeleteCustomerRequest) returns (rpc.customers.v1.DeleteCustomerResponse) {
          option idempotency_level = IDEMPOTENT;
        }
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.customers.v1;

      message GetCustomerRequest { string id = 1; }
      message GetCustomerResponse { string name = 1; }
      message CreateCustomerRequest { string name = 1; }
      message CreateCustomerResponse { string id = 1; }
      message DeleteCustomerRequest { string id = 1; }
      message DeleteCustomerResponse {}
      """

      File.write!(Path.join(rpc_path, "customer_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "messages.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      # The extra space is an artifact of the template and will be removed after
      #  formatting
      expected_impl = ~s"""
      defmodule Rpc.Customers.V1.CustomerService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.Customers.V1.CustomerService.ClientBehaviour
        use RpcClient.Client, service: Rpc.Customers.V1.CustomerService, stub: Rpc.Customers.V1.CustomerService.Stub

        @impl true
        def idempotent_methods do
          MapSet.new([:get_customer, :delete_customer])
        end

        @impl true
        @doc \"\"\"
        Gets a customer by ID.

        Idempotent — retry auto-enabled. Disable with `retry: false`.
        \"\"\"
        @spec get_customer(Rpc.Customers.V1.GetCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.Customers.V1.GetCustomerResponse.t()} | {:error, GRPC.RPCError.t()}
        def get_customer(request, opts \\\\ []) do
          call(request, :get_customer, opts)
        end

        @impl true
        @spec get_customer!(Rpc.Customers.V1.GetCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.Customers.V1.GetCustomerResponse.t() | no_return()
        def get_customer!(request, opts \\\\ []) do
          call!(request, :get_customer, opts)
        end

        @impl true
        @doc \"\"\"
        Creates a new customer.

        No idempotency declared — retry requires explicit `retry: true`.
        \"\"\"
        @spec create_customer(Rpc.Customers.V1.CreateCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.Customers.V1.CreateCustomerResponse.t()} | {:error, GRPC.RPCError.t()}
        def create_customer(request, opts \\\\ []) do
          call(request, :create_customer, opts)
        end

        @impl true
        @spec create_customer!(Rpc.Customers.V1.CreateCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.Customers.V1.CreateCustomerResponse.t() | no_return()
        def create_customer!(request, opts \\\\ []) do
          call!(request, :create_customer, opts)
        end

        @impl true
        @doc \"\"\"
        Deletes a customer.

        Idempotent — retry auto-enabled. Disable with `retry: false`.
        \"\"\"
        @spec delete_customer(Rpc.Customers.V1.DeleteCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.Customers.V1.DeleteCustomerResponse.t()} | {:error, GRPC.RPCError.t()}
        def delete_customer(request, opts \\\\ []) do
          call(request, :delete_customer, opts)
        end

        @impl true
        @spec delete_customer!(Rpc.Customers.V1.DeleteCustomerRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.Customers.V1.DeleteCustomerResponse.t() | no_return()
        def delete_customer!(request, opts \\\\ []) do
          call!(request, :delete_customer, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/customers/v1/customer_service_client_impl.ex"
      )
    end

    test "generates idempotent retry docs without proto comments", %{tmp_dir: tmp_dir} do
      rpc_path = Path.join(tmp_dir, "rpc/items/v1")
      File.mkdir_p!(rpc_path)

      service_proto = """
      syntax = "proto3";

      package rpc.items.v1;

      import "rpc/items/v1/messages.proto";

      service ItemService {
        rpc GetItem(rpc.items.v1.GetItemRequest) returns (rpc.items.v1.GetItemResponse) {
          option idempotency_level = NO_SIDE_EFFECTS;
        }
      }
      """

      rpc_proto = """
      syntax = "proto3";

      package rpc.items.v1;

      message GetItemRequest { string id = 1; }
      message GetItemResponse { string name = 1; }
      """

      File.write!(Path.join(rpc_path, "item_service.proto"), service_proto)
      File.write!(Path.join(rpc_path, "messages.proto"), rpc_proto)

      exec_buf!(tmp_dir)

      expected_impl = ~s"""
      defmodule Rpc.Items.V1.ItemService.Client do
        @moduledoc "Autogenerated by `elixir-rpc-proto-gen-helpers`"

        @behaviour Rpc.Items.V1.ItemService.ClientBehaviour
        use RpcClient.Client, service: Rpc.Items.V1.ItemService, stub: Rpc.Items.V1.ItemService.Stub

        @impl true
        def idempotent_methods do
          MapSet.new([:get_item])
        end

        @impl true
        @doc \"\"\"
        Idempotent — retry auto-enabled. Disable with `retry: false`.
        \"\"\"
        @spec get_item(Rpc.Items.V1.GetItemRequest.t(), RpcClient.ClientBehaviour.call_options()) ::
                {:ok, Rpc.Items.V1.GetItemResponse.t()} | {:error, GRPC.RPCError.t()}
        def get_item(request, opts \\\\ []) do
          call(request, :get_item, opts)
        end

        @impl true
        @spec get_item!(Rpc.Items.V1.GetItemRequest.t(), RpcClient.ClientBehaviour.call_options()) :: Rpc.Items.V1.GetItemResponse.t() | no_return()
        def get_item!(request, opts \\\\ []) do
          call!(request, :get_item, opts)
        end
      end
      """

      assert_content_matches_file(
        expected_impl,
        "#{tmp_dir}/generated/rpc/items/v1/item_service_client_impl.ex"
      )
    end
  end

  defp tmp_dir(context) do
    dir_name =
      "#{inspect(context[:case])}#{context[:describe]}#{context[:test]}"
      |> String.downcase()
      |> String.replace(["-", " ", ".", "_"], "_")
      |> String.replace(~r/[^a-z0-9_]/, "")

    system_temp = System.get_env("TEMP_DIR") || System.tmp_dir!()
    tmp_dir_name = Path.join(system_temp, dir_name)

    File.rm_rf!(tmp_dir_name)
    File.mkdir_p!(tmp_dir_name)

    Map.put(context, :tmp_dir, tmp_dir_name)
  end

  defp assert_content_matches_file(content, file) do
    file_content = file |> File.read!() |> String.trim_trailing("\n")
    assert String.trim_trailing(content, "\n") == file_content
  end

  defp exec_buf!(tmp_dir, opts \\ []) do
    template = Keyword.get(opts, :template, "buf.gen.yaml")

    cwd = File.cwd!()
    buf_gen = Path.join([cwd, "test", template])
    File.cp!(buf_gen, Path.join(tmp_dir, template))
    File.cd!(tmp_dir)

    env =
      [{"ASDF_ELIXIR_VERSION", System.get_env("ASDF_ELIXIR_VERSION")},
       {"ASDF_ERLANG_VERSION", System.get_env("ASDF_ERLANG_VERSION")}]
      |> Enum.reject(fn {_, v} -> is_nil(v) end)

    assert {_, 0} =
             System.cmd("buf", ["generate", "--template", template, tmp_dir],
               stderr_to_stdout: true,
               env: env
             )

    File.cd!(cwd)
  end
end
